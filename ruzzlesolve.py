...
def ruzzlesolve(board_str,board_modifiers):
	...
	def solve(currentword, x, y):
		...
		if (len(currentword) >= MINLENGTH):
			...
			if ("".join(map(lambda v: v[0], V)) in dictionary):
				words.append(V)
		solve(currentword+(board[y][x],),x+1,y+1)
		solve(currentword+(board[y][x],),x+1,y)
		solve(currentword+(board[y][x],),x+1,y-1)
		solve(currentword+(board[y][x],),x,y+1)
		solve(currentword+(board[y][x],),x,y-1)
		solve(currentword+(board[y][x],),x-1,y+1)
		solve(currentword+(board[y][x],),x-1,y)
		solve(currentword+(board[y][x],),x-1,y-1)
